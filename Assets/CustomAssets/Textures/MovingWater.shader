Shader "Unlit/MovingWater"{
	Properties
	{
		...
		_Speed("Cell Speed", Range(0,1)) = 0.3
	}
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float2 uv = i.uv;
				uv *= _CellAmount; //Scaling amount (larger number more cells can be seen)
				...
				for (int y = -1; y <= 1; y++)
				{
					for (int x = -1; x <= 1; x++)
					{
						...
						pointv = 0.5 + _Speed * sin(_Time.z + 6 * pointv);//each point moves in a certain way
